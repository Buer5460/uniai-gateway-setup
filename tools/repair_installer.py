"""Repair the owned v0.8.0 installer entry; keep the gateway payload unchanged.
Build with CPython 3.12. The source binary is pinned by SHA-256.
"""
from __future__ import annotations
import argparse, hashlib, json, marshal, struct, sys, types, zlib
        ...
        if '--silent' not in args:
            try:
                ...
                ctypes.windll.user32.MessageBoxW(None,
                    'UniAI installation failed. See LOCALAPPDATA/UniAIInstaller/Logs/installer.log.',
...
def read_archive(blob):
    ...
    start=cookie+88-size;entries=[];pos=start+tocpos
    while pos<start+tocpos+toclen:
        ...
        entries.append(dict(name=name,raw=blob[start+offset:start+offset+packed],unpacked=unpacked,compressed=compressed,kind=kind));pos+=length
    return blob[:start],entries,pyver,lib,blob[cookie+88:]

def unpack(e): return zlib.decompress(e['raw']) if e['compressed'] else e['raw']

def write_archive(prefix,entries,pyver,lib,trailer):
    payload,toc=bytearray(),bytearray()
    for e in entries:
        name=e['name'].encode('utf-8')+b'\0';length=(18+len(name)+15)//16*16
        toc.extend(struct.pack('!IIIIBc',length,len(payload),len(e['raw']),e['unpacked'],e['compressed'],e['kind']))
        toc.extend(name+b'\0'*(length-18-len(name)));payload.extend(e['raw'])
...
def repair(source,destination):
    if sys.version_info[:2]!=(3,12): raise RuntimeError('Build with CPython 3.12')
    original=source.read_bytes();digest=hashlib.sha256(original).hexdigest()
    if digest!=EXPECTED: raise ValueError('Input differs from reviewed v0.8.0')
    pe=struct.unpack_from('<I',original,0x3c)[0];opt=pe+24
    cert_offset,cert_size=struct.unpack_from('<II',original,opt+112+8*4)
    if cert_offset or cert_size: raise ValueError('Signed input requires source rebuild and authorized re-signing')
    ...
    if 'Installer' in oldmain.co_names: raise ValueError('Missing-entry defect no longer matches')
    ...
    env={'argparse':argparse,'sys':sys,'Path':Path,'__file__':str(source.parent/'product_install.py'),'payload_zip':lambda:Path('stub'),'Installer':DummyWizard}
    ...
    report={'input_sha256':digest,'output_sha256':hashlib.sha256(output).hexdigest(),'output_bytes':len(output),'bundle_python':'3.12','changed_entries':['product_install'],'payload_sha256':before_payload,'gateway_payload_unchanged':True,'no_argument_before':before,'no_argument_after_spy_result':after,'evidence_level':'entry-function test with stub GUI; Windows end-to-end is separate','signature':'unsigned input; repaired binary remains unsigned'}
